...
**Last updated:** 2026-08-01 (Phase 1 of the billing/beneficiary fix work order resolved live — see "Session Update — 2026-08-01" below. Identity Sprint (07-26/27) and Billing + Beneficiary Commitments (07-31) both built since the 07-25 session but not fully verified; a phased fix work order is being worked through one phase at a time with live verification before each greenlight, given this project's repeated "documented as done ≠ actually done" history.)
...
**Database:** Supabase (schema deployed, 27 migrations — 010 corrected by 011, 012/013's linking triggers hotfixed by 020; see below)
...
| **Billing + Beneficiary Commitments (Phases 1–7)** | ⚠️ Built, **not** verified-working — see below | Migrations 021–027 applied live. E2E pass (2026-07-31) found: Phase 4 (public commitment notice) confirmed broken on all surfaces tested, **still unfixed**; Phase 5 (fulfillment evidence) fully blocked — no UI path exists to advance an event to `completed`; Phase 6 (ledger) has real gaps (no expense edit, expenses not wired to running total, no donation update/delete); Phase 7 (billing panels) partially found on closer look — `SuperAdminDashboard.jsx` has inline billing-management UI, not confirmed whether it was there all along or is newly reconciled. Phases 2–3 have known open issues (verification labeling, inconsistent beneficiary-detail visibility). Fix work order in progress, one phase at a time — Phase 1 (a *separate*, unrelated events-RLS bug found while starting this work — blocked creating a second event under any org, not connected to Phase 4's render bug) resolved 2026-08-01 via migration 028. |
...
**As of 2026-07-31, this checklist was not picked back up** — that session's testing focused on the billing/beneficiary work instead (see below). As of 2026-08-01, still not picked back up. Worth deciding whether to close this out before or after the billing/beneficiary fix work order, given it touches live auth for every non-super-admin role.

---

## ⚠️ Session Update — 2026-07-31: Billing + Beneficiary Commitments built, then E2E-tested — multiple gaps found

Between the 07-25 session and 07-31, `CC_WORK_ORDER_billing_and_beneficiary.md` was executed against `FEATURE_SPEC_ledger_platform_fee_payments.md` and `FEATURE_SPEC_billing_and_beneficiary_commitments.md` — Phases 1–7 (beneficiaries + verification stub, commitment creation + publish gate, public commitment-notice surfaces, post-event fulfillment evidence, ledger, billing schema + manual-status UI). Migrations 021–027 written and manually applied live.

**End-to-end verification (`billing_beneficiary_e2e_checklist.md`) found this feature substantially less complete than "Phases 1–7 built" implies** — another instance of this project's recurring "documented as done ≠ actually done" pattern (see 2026-07-22 process note below), only caught via direct live testing.

**Confirmed bugs:**
- **Phase 4 — public commitment notice does not render anywhere.** Tested on two independently-built events (different orgs, different beneficiaries, both with published commitments) — the "Cause" section is absent from the public event page in both cases. The other three required surfaces (team registration, volunteer application, invite-acceptance) not yet individually confirmed broken, but suspect given a likely shared root cause. **Still unfixed as of 2026-08-01.**
- **Phase 6 — ledger has real functional gaps.** Expenses can be added/deleted but not edited, and don't affect the running total at all — only donations do. Donations can be created/read but not updated/deleted. Tab visibility and create/delete permission-gating all confirmed correct.

**Fully blocked, not a bug per se:**
- **Phase 5 — post-event fulfillment evidence is entirely untestable.** No UI control anywhere, including super_admin, advances an event's `status` past `registration_open`. Not calendar-driven — no scheduled-job infrastructure exists anywhere in this codebase.

**Appears not to be deployed at all (initial finding, partially reconciled 2026-08-01 — see below):**
- **Phase 7 — billing UI not found** in the initial pass.

**Still-open items:**
- Phase 2: fake verification stub marks a format-valid-but-fake registration number "Verified," overstating confidence. Needs a labeling decision, and separately a manual-evidence verification path for org admins.
- Phase 3: an org_admin can see beneficiary registration details on one event but not another, inconsistently — not yet root-caused. Also: no way to edit a saved beneficiary commitment, even in `draft`.
- Second event under an existing org throws an RLS `42501` — **root-caused and fixed 2026-08-01, see below.** This turned out to be an unrelated bug (events RLS, not beneficiary/billing-specific) surfaced while starting this fix work.

**Plan set 07-31, still in effect:** a phased fix work order (`CC_WORK_ORDER_billing_beneficiary_fixes.md`) covers all of the above. Each phase is sent to CC, live-verified by the project owner, and only then greenlit into the next phase — not batched — given how much of "Phases 1–7 built" turned out not to actually work once tested.

---

## ⚠️ Session Update — 2026-08-01: Phase 1 fixed (real root cause found), role-model clarification, nav gaps backlogged

**Phase 1 of the fix work order — resolved, not just diagnosed.** Root cause fully confirmed live via direct SQL Editor investigation (session-faked RLS testing with `SET LOCAL ROLE`/`SET LOCAL request.jwt.claims`, plus Postgres logs): `events` had no `SELECT` policy covering an org_admin reading their own org's **draft** events. Supabase's JS client always appends `RETURNING` to `.insert()` calls, and Postgres checks `RETURNING` visibility against `SELECT` policies — so **any** org_admin creating **any** new event (always starts `status='draft'`) hit this `42501` error, not just "second events." Confirmed precisely: identical insert with `RETURNING` removed succeeded cleanly; with `RETURNING` present it failed identically to the original bug report, in the same simulated session. Also explains why it was never caught earlier — `organizations` INSERT is `super_admin`-only, so an org's first-ever event is always created by a super_admin (passes every SELECT check trivially via `is_super_admin()`); an org_admin only exercises the create-event path starting with their org's *second* event, which is exactly the scenario that surfaced this. Unrelated to Phase 4's still-open public-commitment-notice render bug above — separate bug, don't conflate.

**Fix applied:** migration `028_fix_org_admin_events_select.sql` — new `SELECT` policy `"Org admin read own events"` granting `is_super_admin() OR is_org_admin_for(org_id) OR is_event_admin_for(id)`. Applied live 2026-08-01, confirmed by CC as non-conflicting with existing policies (Postgres ORs multiple permissive SELECT policies). **Live re-verification by the project owner (second event under an existing org, plus a sanity check on a brand-new org's first event) still needs to happen before Phase 2 is greenlit** — do not treat this as fully closed until that's done, consistent with the one-phase-at-a-time process.

**Role-model clarification (worth remembering, easy to mis-read):** `super_admin` is **platform-wide** (`org_id IS NULL`) — sees/manages every org and event on the platform. It is not "the owner of the org I created." The role that means "full owner of one org" is `org_admin`. This came up because the project owner initially read CC's Phase 1 report ("only super_admin can insert into organizations") as implying a self-serve-signup UX where a new user becomes `super_admin` of their own org — corrected before it shaped any design decisions. See the self-serve org creation backlog item in the Proposed Schedule below for where this actually lands.

**Phase 7 billing UI, partially reconciled:** while investigating unrelated code, found `SuperAdminDashboard.jsx` does contain inline billing-management UI (org subscription/event billing creation, status changes) — not a separately-named `BillingPanel` component as the original work order described, but functionally present. Not yet confirmed whether this was there all along and the E2E tester simply didn't recognize it, or whether it was added since. Worth clarifying directly with CC in Phase 4 of the fix work order rather than assuming either way.

**Two new items backlogged, not built:**
- No in-app entry point to create a second event (`/wizard` only reachable by direct URL after first visit) — see Known Issues #18.
- No cross-org/cross-event navigation for super_admin/org_admin — a "Move To" fold-out nav concept was discussed (rename Build→Manage, nested org→event tree, "make new event"/"Make New Org" entries) and sized at a rough level (mostly small/medium pieces, one piece of real new logic for letting super_admin target an existing org in the Wizard). Not started — see Known Issues #19 for the full concept and sizing notes. Sequenced after the current fix work order, same backend-first reasoning as the self-serve org creation item.
...
│       ├── 022_beneficiary_commitments_public_view.sql  # written 07-31, NOT actually applied until 08-01 — see Phase 2 fix
│       ├── 023–024_*.sql                    # ledger tables + fulfillment tracking (billing/beneficiary work)
│       ├── 025_billing_tables.sql
│       ├── 026_*.sql
│       ├── 027_org_subscriptions_created_at.sql
│       ├── 028_fix_org_admin_events_select.sql # fixes RETURNING-vs-SELECT-policy bug blocking event creation
│       └── 029_fundraising_reconciliation.sql   # events.fundraising_reconciled_amount, Phase 5 ledger fix scope expansion
├── src/
│   ├── main.jsx
│   ├── index.css
...
12. **No billing/payment schema of any kind.** ~~Marketing page pricing is display-only.~~ **Superseded 2026-07-31** — a billing schema now exists (migrations 025/027) with inline management UI in `SuperAdminDashboard.jsx`. Centralized-vs-decentralized payment collection remains an open business/legal question (see Architecture Decisions above) not yet resolved.
13. ~~**Public beneficiary commitment notice does not render.**~~ **RESOLVED 2026-08-01** — root cause was migration 022 never actually applied (written but not run, same failure class as migration 010's history). Fixed by applying it live; view/grants/data all confirmed correct after. Public event page live-verified working (logged out, incognito). The other three surfaces (team registration, volunteer application, invite-acceptance) are wired to the same now-working code path per CC's trace but not yet each individually click-tested — worth a quick pass before fully trusting this closed.
...
15. ~~**Ledger (Phase 6 of billing/beneficiary work) has functional gaps.**~~ **RESOLVED 2026-08-02** — expense/donation edit added, donation delete added, running total now auto-derived from the ledger (was previously fully manual — a premise correction CC flagged before building, not assumed). Scope expanded on project owner's direction to add a separate Event-Day Reconciliation figure (migration 029) and relabel the thermometer "Donations Received." Fully live-verified including the full add/edit/delete recompute cycle across both admin and public views. See Session Update 2026-07-31 for original gap, Phase 5 of the fix work order for the resolution.
16. ~~**Beneficiary commitments can't be edited once created**, even while still `draft`.~~ **RESOLVED 2026-08-02** — draft edit built and live-verified. **Related finding, accepted as a known limitation rather than fixed:** published-commitment immutability is UI-level only, not RLS-enforced — a direct SQL `UPDATE` against a published commitment's text succeeds with no error (confirmed live). Accepted deliberately: matches this project's existing pattern elsewhere, and only admins with legitimate access to that org could exploit it. Revisit if the fulfillment/beneficiary feature ever needs a stronger "published = literally immutable" guarantee (e.g. for external auditors).
17. ~~**Second event under an existing org throws an RLS `42501` on `events` insert.**~~ **RESOLVED 2026-08-01** — root-caused and fixed via migration 028. See "Session Update — 2026-08-01" for the full diagnosis; root cause was a missing `SELECT` policy on `events` for org_admins reading their own org's draft events, which broke Supabase's default `RETURNING` clause on insert. Live re-verification by the project owner still pending.
18. **No in-app entry point to create a second/additional event.** The only way to reach `/wizard` today is the very first visit (`/` redirects there when no default event is configured) — after that, there's no button/link anywhere in `AdminDashboard` or `SuperAdminDashboard` that takes a logged-in `org_admin`/`super_admin` back to it. Only reachable by typing the URL directly. Backlogged alongside item 19 below since they're the same underlying gap (no cross-org/cross-event navigation for admins).
19. **No cross-org/cross-event navigation for super_admin or org_admin.** A super_admin has no way to browse from one org/event's admin dashboard to another without knowing URLs directly; same for an org_admin moving between their own org's events. Concept discussed 2026-08-01, worth building as one feature alongside item 18:
    - Rename the "Build" tab to "Manage" (trivial)
    - Add a "Move To" nested fold-out control: for `super_admin`, every org → its events (opens that event's admin dashboard) → a "make new event" entry per org (opens Wizard pre-scoped to that org) → a top-level "Make New Org" entry (opens the plain Wizard, unscoped). For `org_admin`, the same tree scoped to just their one org.
    - **Sizing:** mostly small/medium pieces (rename, a role-aware nested query combining existing list patterns, a fold-out UI component) — the one piece needing real new logic is letting a `super_admin` launch the Wizard pre-scoped to a specific *existing* org they picked from the tree (today the Wizard only auto-skips org-creation for `org_admin` via their own `adminUser.org_id`; a super_admin targeting an arbitrary org needs the same skip-org-creation behavior but parameterized by a chosen org rather than their own).
    - Not started. Spec this properly before building — sequence after the current billing/beneficiary fix work order, same reasoning as the self-serve org creation item below (backend-first).
20. **Identity Sprint (07-26/27) verification checklist never completed.** Migrations 017–020 applied live and confirmed functionally working (QR generation succeeds), but the full manual checklist (treasurer invite retry, first-time captain check-in on a real device, first-time volunteer login, per-role permission boundaries, re-confirming migration 019 specifically) was never run. See Session Update 2026-07-26/27.
21. ~~**`/super-admin` route doesn't work live**~~ **RETRACTED 2026-08-02 — false alarm, not a real bug.** Confirmed working perfectly once navigated to correctly (`http://localhost:3000/super-admin`, not `/e/:eventId/super-admin`, which was the actual URL being tried and correctly 404s — no route exists at that path, nor should one). No RLS gap, no deploy gap, no code issue. Retained here only as a note-to-self: the earlier navigation-gap issues (#18/#19, no in-app link to `/super-admin` or `/wizard`) are exactly what made this kind of URL slip easy to make and hard to notice — another point in favor of building that nav.

---

## Proposed Schedule (Next Session)

**Immediate priority, superseding everything below until it's resolved:** work through `CC_WORK_ORDER_billing_beneficiary_fixes.md` one phase at a time — send each phase to CC, live-verify the result, only then greenlight the next. Do not batch phases. This doc is updated after each phase with a pass/fail, not just at the end. Phase 1 (a separate events-RLS bug, not part of the original billing/beneficiary work) is fixed as of 2026-08-01 (migration 028) — live re-verification (second event under an existing org, plus a first-event-by-super_admin sanity check) still needs to happen before Phase 2 is greenlit.

**Competing priority, not yet scheduled — flagging rather than deciding:** the Identity Sprint's own verification checklist (07-26/27) was never completed either, and it touches live auth for every non-super-admin role including the highest-risk RLS migration in the project (019). Worth a deliberate decision on sequencing — finish that checklist before continuing the billing/beneficiary fixes, interleave them, or explicitly accept the risk of leaving it unverified a while longer — rather than letting it default to "whichever gets mentioned last."

1. **Phase 1** — ~~diagnose~~ ✅ **fixed and live-verified 2026-08-01** — the second-event RLS block. Verified via manual `admin_users` reclassification (role=org_admin, event_id=NULL) since `/super-admin`'s proper invite flow is currently inaccessible (see Known Issues #21) — confirmed accessing both first and second events works with no RLS errors.
2. **Phase 2** — ✅ **fixed and live-verified 2026-08-01.** Root cause: migration 022 (public commitment-notice view) never actually applied — applied it live, confirmed working on the public event page. Other three surfaces (team reg, volunteer app, invite-acceptance) not yet individually click-tested but share the same now-fixed code path.
3. **Phase 3** — ✅ **fixed and live-verified 2026-08-02.** Extended the existing `EventStatusCard` (Publish tab) with a sequential advance button through the full lifecycle (`draft → registration_open → registration_closed → game_day → completed → archived`); reused existing `events.updateStatus()`, no new API/RLS needed. Charity-commitment gate preserved on the `draft → registration_open` step only. Live-verified full sequence including reaching `archived`. **Bonus:** while testing, also live-verified the entire original Phase 5 fulfillment-evidence lifecycle that had been sitting untestable since 07-31 — submit → super_admin review → confirm all confirmed working end to end (`fulfillment_status` correctly flips `submitted → confirmed`, `reviewed_by`/`reviewed_at` populate). One false alarm along the way: `/super-admin` briefly looked broken (404) but was a URL-typing slip (`/e/:eventId/super-admin` instead of `/super-admin`) — not a real bug, retracted from Known Issues.
4. **Phase 4** — ✅ **fixed and live-verified 2026-08-02, entirely via direct testing, zero CC involvement needed** — a first for this work order. Confirmed: super_admin can create org subscriptions and per-event billing rows, status cycles correctly, `paid` auto-stamps `paid_at`, org_admin's Team tab renders the same data read-only with no edit controls, and — the specific bug the earlier `created_at` fix targeted — creating a second subscription for the same org correctly surfaces the newest one on the org_admin's card, not an arbitrary one. Phase 7 billing UI is real and working, just inline in `SuperAdminDashboard.jsx` rather than a separately-named component. **New scope surfaced during this verification:** no pricing logic, no start/end dates, and no freeze/thaw access model exist yet — see `FEATURE_SPEC_billing_pricing_and_freeze_model_CONCEPT.md` (concept captured 2026-08-02, not build-ready) for the full model discussed. Not part of this fix work order — separate future feature.
5. **Phase 5** — ✅ **fixed and fully live-verified 2026-08-02.** Expense edit, donation edit+delete, running total auto-derived from the ledger (was fully manual before — real premise correction, not assumed), plus a scope expansion: Event-Day Reconciliation figure (migration 029) and thermometer relabeled "Donations Received." Full add/edit/delete recompute cycle walked through step by step against both admin and public views — all passed cleanly.
6. **Phase 6** — ✅ **fixed and live-verified 2026-08-02.** Draft editing built and confirmed working, edit-then-publish confirmed in one step. Real finding along the way: published-commitment immutability is UI-level only (a direct SQL UPDATE against a published row succeeds, confirmed live) — deliberately accepted as a known, documented limitation rather than fixed with an RLS follow-up. See Known Issues #16.
...
**Once the above is fully closed out and re-verified, resume the pre-existing backlog:**
...
1. **Marketing page (`/`) + `/your_events` build-out** — per "Architecture Decisions — 2026-07-23" above. Suggest sequencing as: (a) marketing page with static pricing content, (b) `/your_events` for admin roles only first (reuses existing role data), (c) branding system split (platform skin vs. event skin), (d) non-admin identity/link mechanism last, since it's the least-defined piece and has real open design questions.
   - **New, 2026-08-01: self-serve org creation.** Surfaced while diagnosing the Phase 1 events-RLS bug — `organizations` INSERT is currently locked to `super_admin` only (migration 010), and the only place an org gets created today is inside the Wizard, run by an already-provisioned `super_admin`. There is no path today for an uninvited visitor to sign up and get their own org. Project owner has a concept in mind for how this should work (self-serve signup → new org → user becomes `org_admin` of that one org, **not** `super_admin` — `super_admin` is platform-wide and must never be handed out via self-serve signup, or every customer could see every other customer's data) but has deliberately deferred building it until the backend is fully functional — payment/signup UX should be layered onto a working model, not built in parallel with open backend bugs. Revisit once the billing/beneficiary fix work order (above) is closed out.
   - **New, 2026-08-01: cross-org/cross-event navigation ("Move To" nav).** See Known Issues #18/#19 above for the full concept and sizing. Same backend-first sequencing as self-serve org creation.
2. **Entitlements/CRUD matrix** — design doc first (role × resource × operation × scope), then map onto existing RLS scope functions. Still deferred, but worth doing before the identity/marketing work above makes the permission surface bigger — and now also relevant to the billing/beneficiary RLS gaps found 07-31/08-01.
3. **Step 5 — Serverless functions**: Twilio (OTP SMS — note: superseded for captain identity by the QR/magic-link redesign, see Identity Sprint session update; Twilio may still be relevant for other notification use cases, worth revisiting whether it's needed at all) and Stripe (payments) — needed for real payment processing to actually function. **Note:** "Stripe" here is event-level payments only (a team paying its entry fee) — this is a different, smaller thing than the org-level SaaS billing question raised in Architecture Decisions, which needs a legal/business decision first and is not in scope for this step. Also distinct from the billing/beneficiary work's Phase 7, which is Cocomo's own subscription revenue (currently hand-set, no real Stripe integration either).
4. **Step 6 — Artifact generation engine**: real schedule/run-sheet/resource-directory generation for the Publish tab.
5. **New, 2026-08-02: billing pricing model + freeze/thaw access control.** Surfaced while verifying Phase 4 — the existing `org_subscriptions`/`event_billing` schema handles manual status-cycling correctly but has no pricing logic, no start/end dates, and no concept of what happens to access when payment lapses or an event finishes unpaid. Full concept captured in `FEATURE_SPEC_billing_pricing_and_freeze_model_CONCEPT.md` — per-day event pricing, recurring-event-as-contract billing, two-scope freeze (per-event for paid events, org-wide for subscriptions) with cumulative super_admin-grantable grace periods, two-severity freeze (soft: admin locked/public live, hard: public down too), and a 7-day trial-to-freeze onboarding funnel tied to the still-unbuilt self-serve org creation item above. Several open questions explicitly flagged as undecided in that doc (grace-period tier criteria, soft→hard trigger, recurring-series data model) — needs those resolved and a real `FEATURE_SPEC` before this becomes a work order. Directly relevant to item 3's Stripe integration when that's eventually built — this is the pricing/access logic Stripe would need to enforce, not a replacement for it.
...
## Database (supabase/schema.sql + 29 migrations)

22 core tables from the original schema + `activity_log`, `invites`, `admin_lock_queue`, plus `beneficiaries`, `event_beneficiary_commitments`, `billing_plans`, `org_subscriptions`, `event_billing`, and the ledger tables (`transactions`/`fan_donations`/`expenses`) from the billing/beneficiary work. RLS org/event-scoped per admin since migration 010, further narrowed per-role for treasurer/volunteer_coord/referee/control_desk by migration 019 (see Identity Sprint session update — verification still incomplete), with migration 028 fixing a `SELECT`-policy gap that blocked org_admin event creation via `RETURNING`. Realtime enabled on `matches`, `pool_standings`, `playing_areas`, `teams`, `announcements`, `playing_area_queue`, `admin_lock_queue`. Key triggers/RPCs: reconciliation code generation (`SECURITY DEFINER`, atomic), invite-signup → admin_users auto-creation, pool standings recomputation, fundraising totals, volunteer role fill counts, QR check-in auth linking (migration 018), captaincy transfer/team check-in RPCs (migration 019).

## ⚠️ Session Update — 2026-08-03: Billing/beneficiary work order closed, nav/wizard work order closed, next target picked

**`CC_WORK_ORDER_billing_beneficiary_fixes.md` — fully closed.**
- Phase 7 (inconsistent beneficiary-detail visibility) — ✅ **resolved 2026-08-03, root cause found directly, no CC involvement needed.** Not an RLS bug — `beneficiaries` is intentionally org-scoped (reusable across an org's events, confirmed correct by design) and `event_beneficiary_commitments` is correctly event-scoped in both the RLS and the query code (`commitments.listByEvent`, filtered on `event_id`, not `org_id`). The actual cause: the Beneficiary Commitment panel is gated on that specific event's `is_charity` flag (`config.cause.isCharity`), and the second event simply hadn't been flagged as a charity event. Confirmed live: creating a third event in the same org with "charity event" checked shows the commitment box correctly. Working as designed.
- Phase 8 (fake-verification labeling decision) — ✅ **decided 2026-08-03.** Badge wording stays "Verified" as-is (overstatement accepted for now). A manual evidence-based verification path (org admin submits proof, super_admin reviews — same pattern as fulfillment evidence) is approved in principle but **not built** — needs its own spec before it's a work order. New backlog item.

**`CC_WORK_ORDER_nav_and_wizard_scoping.md` (new, covering Known Issues #18/#19) — written, sent to CC, fully closed 2026-08-03.** Both phases passed live verification with no issues found:
- Phase 1 — `super_admin` can now target an existing org in the Wizard via `/wizard?orgId=<uuid>`, skipping org creation the same way the org_admin path already did. `createTournament()` signature extended to accept an explicit target org id, kept distinct from `adminUser.org_id` in the code.
- Phase 2 — "Build" tab relabeled "Manage" (cosmetic only, `ROLE_TABS`/internal id unchanged). New `OrgEventNav` fold-out component, `super_admin`/`org_admin` only: org → events → "+ New Event" (using Phase 1's `orgId` param for super_admin) → "+ New Org" for super_admin.

Known Issues #18 and #19 can now be marked resolved.

**Next target, chosen deliberately given the goal is a soft launch to hand-picked real orgs (not public self-serve yet):** the Identity Sprint verification checklist (`phase6_verification_checklist.md`), flagged as an unscheduled risk since 07-27 and never picked back up across the 07-31/08-01/08-02 sessions. Reasoning: it's pure verification of already-built work (2 of the items already confirmed), not new development, and it's the one item that touches live auth for every non-super-admin role via the highest-risk RLS migration in the project (019) — the right thing to close before real people start relying on those role boundaries. Self-serve org creation, the marketing page, and the artifact generation engine are deliberately deferred past this soft launch — none are needed to onboard hand-picked orgs directly.

**Remaining checklist items extracted to `IDENTITY_VERIFICATION_REMAINING.md`** for quick execution — manual live-app testing only, no CC involvement needed unless a failure surfaces (in which case, note whether it traces to migration 019's RLS trim or Phase 6b's UI-hiding pass before reporting back, since the fix differs).